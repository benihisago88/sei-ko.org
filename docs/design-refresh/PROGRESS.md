...
- **状態**: 完了
- **日付**: 2026-08-15
- **キャッシュバスター**: styles.css `?v=20260815.1`（4ページで統一）
- **変更ファイル**:
  - `styles.css` — `--accent-strong`を追加し、CTA、ヒーロー強調、補助ラベル、選択状態、サービス番号、ケース・タイムライン、ガイドのパンくず、リーガルリンクを可読性の高いアクセント色へ統一。記事CTA内のアイブラウは暗色背景で十分なコントラストとなるよう`--lime`を指定。
  - `index.html` / `guide.html` / `privacy.html` / `404.html` — styles.cssのキャッシュバスターを`?v=20260815.1`へ更新。
  - `docs/design-refresh/contrast-audit-20260815.md` — 修正対象とローカル監査結果を記録。
- **検証結果**:
  - Lighthouse Accessibility: index 100 / guide 100 / privacy 100 / 404 100（ローカル `python -m http.server 8778`）。
  - `/` / `/guide.html` / `/privacy.html` / `/404.html` はすべて200。トップページは1280px相当で横スクロールなし（`scrollWidth === clientWidth`）。
  - トップ画面でCSS `?v=20260815.1` の取得、CTA・ヒーロー強調・アイブラウの計算済み色を確認。コンソール上の資産読込失敗なし。
  - `node --check script.js`、`git diff --check`は通過。
  - ローカルサーバーは`.htaccess`を解釈しないため、CSP・圧縮・キャッシュ・リダイレクトの本番確認は未実施。
- **次フェーズへの申し送り**: 今回は既存コピーと情報構造を変えず、可読性に限定して修正した。トップの情報設計、料金の判断材料、運営方針・相談例などは、ユーザー確認済みの事実を得てから別スコープで実装する。
- **スコープ外で気づいた点**: アーカイブ診断で見つかった旧コピー混在は、GitHubリポジトリのクリーンな作業コピーには再現しなかった。デプロイ用アーカイブを作る際は、Git・IDE・作業ツリーを含めない運用を別途徹底する。
